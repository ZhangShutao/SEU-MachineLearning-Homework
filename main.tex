...
\begin{document}
	...
	\begin{frame}{总目录}
		第\ref{part:rl-and-planning}部分~强化学习与规划（张舒韬）\\
		第\ref{part:her}部分~事后经验回放（刘翔）\\
		第\ref{part:dqn}部分~Deep Q Network（赵倩隆）\\
		第\ref{part:fps}部分~DRQN与FPS游戏（吴江恒）\\
	\end{frame}
	...
	\begin{frame}{HER}{算法描述}
		...
		\begin{algorithm}[H]
			...
			\caption{事后经验回放算法（Hind Experience Replay，HER）}\label{alg:her}
		\end{algorithm}
	\end{frame}
